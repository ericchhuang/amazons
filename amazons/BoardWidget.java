...
class BoardWidget extends Pad {
    ...
    private void drawGrid(Graphics2D g) {
        ...
        for (int k = 0; k <= BOARD_SIDE; k += 2 * SQUARE_SIDE) {
            for (int j = 0; j <= BOARD_SIDE; j += 2 * SQUARE_SIDE) {
                g.fillRect(k, j, SQUARE_SIDE, SQUARE_SIDE);
            }
        }
        for (int k = SQUARE_SIDE; k <= BOARD_SIDE; k += 2 * SQUARE_SIDE) {
            for (int j = SQUARE_SIDE; j <= BOARD_SIDE; j += 2 * SQUARE_SIDE) {
                g.fillRect(k, j, SQUARE_SIDE, SQUARE_SIDE);
            }
        }
        ...
        for (int k = 0; k <= BOARD_SIDE; k += SQUARE_SIDE) {
            g.drawLine(k, 0, k, BOARD_SIDE);
        }
        for (int k = 0; k <= BOARD_SIDE; k += SQUARE_SIDE) {
            g.drawLine(0, k, BOARD_SIDE, k);
        }
    }
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        drawGrid(g);
        for (Square queen : _board.whites()) {
            drawQueen(g, queen, Piece.WHITE);
        }
        for (Square queen : _board.blacks()) {
            drawQueen(g, queen, Piece.BLACK);
        }
    }
    ...
    private void drawQueen(Graphics2D g, Square s, Piece piece) {
        g.drawImage(piece == WHITE ? _whiteQueen : _blackQueen,
                    cx(s.row()) + 2, cy(s.col()) + 4, null);
    }

    /** Handle a click on S. */
    private void click(Square s) {
        _commands.add("d1 d5 d6");
        repaint();
    }
